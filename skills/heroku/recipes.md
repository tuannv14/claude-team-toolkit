# /heroku — recipes (load on demand)

> Loaded by SKILL.md only when user invokes a specific dispatch verb.
> Keep SKILL.md lean; long curl recipes live here.
...
'
```
Add `--reveal` to show all in full (require explicit user request).
...
## Dispatch — mutating (require ctt_confirm + ctt_audit_log)
...
### `rollback <app> <version>` — DESTRUCTIVE, typed confirm
```bash
APP=$(heroku_app "$1")
...
### `promote <pipeline-id>` — DESTRUCTIVE, typed confirm
```bash
...
### `destroy <app>` — IRREVERSIBLE, type app name
```bash
APP=$(heroku_app "$1")
